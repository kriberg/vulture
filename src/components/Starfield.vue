...
<script>
  /*
   * Starfield shamelessly stolen from https://codepen.io/bigsweater/pen/MyqZdQ
   */
  ...
  Vue.component('starfield', {
    ...
    computed: {
      colors () { // Use one.color to add opacity based on state
        return {
          fill: this.hyperspace ? colour(this.bgColor).alpha(this.opacity).cssa() : this.bgColor
        }
      },

      compColors () {
        return {
          stars: colour(this.starColor).hex(),
          bg: colour(this.bgColor).hex()
        }
      },
      ...
      starz () {
        ...
        let sf = {
          ...
          canvas () {
            ...
            // Set up context
            let c = document.getElementById('starfield-canvas')
